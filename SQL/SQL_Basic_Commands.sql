...
--Retrieve customers from Almere or Amersfoort.
select cust_first_name || ' ' || cust_last_name , cust_city
from sh.customers
where cust_city IN ('Almere','Amersfoort')

--Find customers who do not have a credit limit.
select cust_first_name || ' ' || cust_last_name , cust_credit_limit
from sh.customers
where cust_credit_limit is null

--List customers whose phone number starts with "487".
select cust_first_name || ' ' || cust_last_name , cust_main_phone_number
from sh.customers
where cust_main_phone_number like '487%'

--Find married customers with income level `"Medium"`.
...
--Get customers whose last name starts with "G".
select cust_first_name || ' ' || cust_last_name
from sh.customers
where  cust_last_name like 'G%'

--Find customers with city_id = 51057.
select cust_first_name || ' ' || cust_last_name , cust_city_id
from sh.customers
where  cust_city_id = '51057'

--Retrieve all customers who are valid (`CUST_VALID = 'A'`).
select cust_first_name || ' ' || cust_last_name , cust_valid
from sh.customers
where  cust_valid = 'A'

--Find customers whose effective start date (`CUST_EFF_FROM`) is after 2020.
select cust_first_name || ' ' || cust_last_name, cust_eff_from 
from sh.customers
where cust_eff_from > TO_DATE('2020-12-31', 'YYYY-MM-DD')

--Retrieve customers whose effective end date (`CUST_EFF_TO`) is before 2021.
select cust_first_name || ' ' || cust_last_name , cust_eff_to
from sh.customers
where  cust_eff_to < TO_DATE('2020-12-31', 'YYYY-MM-DD')

--Find customers with credit limit between 5,000 and 9,000.
select cust_first_name || ' ' || cust_last_name , CUST_CREDIT_LIMIT
from sh.customers
where  CUST_CREDIT_LIMIT between '5000' and '9000'

--Get all customers from country_id = 101.
select cust_first_name || ' ' || cust_last_name ,country_id
from sh.customers
where country_id = 101

--Find customers whose email ends with `"@company.example.com"`.
select cust_first_name || ' ' || cust_last_name , CUST_EMAIL
from sh.customers
where  CUST_EMAIL like '%@company.example.com'

--List customers with `CUST_TOTAL_ID = 52772`.
select cust_first_name || ' ' || cust_last_name , cust_total_id
from sh.customers
where cust_total_id = '52772'

--Find customers with `CUST_SRC_ID` in (10, 20, 30).
select cust_first_name || ' ' || cust_last_name , cust_src_id
from sh.customers
where cust_src_id in ('10','20','30')

--Retrieve customers who either do not have email or do not have a credit limit.
...
-- Find states with fewer than 50 customers.
select cust_state_province, count(*)
from sh.customers 
group by cust_state_province having count(*) < 50

--Calculate the average credit limit of customers in each city.
